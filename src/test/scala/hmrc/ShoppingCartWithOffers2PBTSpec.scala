...
class ShoppingCartWithOffers2PBTSpec extends AnyFreeSpec with Matchers with ScalaCheckDrivenPropertyChecks {
  ...
  s"checkout should calculate the total cost when the list of items is provided" in {
    forAll { items: List[Item] => ShoppingCartWithOffers2.checkout(items) shouldBe calculateCost(items) }
  }
  ...
  private def calculateCost(items: List[Item]) = {
    ...
    (applesPrice + orangesPrice + bananasPrice).print
  }
}
